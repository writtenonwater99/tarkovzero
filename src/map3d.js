...
export async function createView3d(container, mapData, src) {
  ...
  const dynamicLayers = () => {
    ...
    return [
      ...
      new IconLayer({ id: 'player-figures', data: players, getPosition: (p) => P([p.last.x, p.last.z], Math.max((p.last.y ?? 0) + 0.2, H(p.last.x, p.last.z) + 0.3)), iconAtlas: soldierAtlas.canvas, iconMapping: soldierAtlas.mapping, getIcon: (p) => p.color, getSize: 34, sizeUnits: 'pixels', billboard: true, parameters: OVERLAY, coordinateSystem: COORDINATE_SYSTEM.CARTESIAN, updateTriggers: { getPosition: players.map((p) => p.last) } }),
      new IconLayer({ id: 'players', data: players, getPosition: (p) => P([p.last.x, p.last.z], (p.last.y ?? 0) + 0.2), iconAtlas: arrowAtlas.canvas, iconMapping: arrowAtlas.mapping, getIcon: (p) => p.color, getSize: 6, sizeUnits: 'meters', sizeMinPixels: 14, sizeMaxPixels: 26, billboard: false, getAngle: (p) => -((p.last.yaw ?? 0) + (mapData.coordinateRotation ?? 0)), pickable: true, parameters: OVERLAY, coordinateSystem: COORDINATE_SYSTEM.CARTESIAN, updateTriggers: { getPosition: players.map((p) => p.last), getAngle: players.map((p) => p.last) } }),
