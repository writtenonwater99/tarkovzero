...
- **An extract owns its name (QA M1/M3)** — the badge draws the name as a caption, so a place label with the same text renders the POI twice. `ownedByExtract()` in main.js reads the LIVE marker set (eleven rows collide across the three shipped maps) and hides the place label in 3D, in 2D (`placeLabelsLayer`'s `hidden` predicate, consulted per clip because the marker data lands after the layer is built) and in the omnibox index.
