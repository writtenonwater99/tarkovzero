...
//
// THE CI FAIL-OPEN, AND WHAT CLOSED IT (2026-09-02)
//
// Check 4 used to compare `dist/` against an index built by WALKING the capture roots. Vercel
// builds from a clean checkout: `.local-game-derived/` and `.local-candidates/` are git-ignored and
// `.vercelignore`d, so on the one deployment that matters both roots were ABSENT,
// `collectSubtreeHashes()` returned an empty map, and a raw capture renamed and committed under
// `public/assets/` matched nothing and shipped. The protection was inert exactly where it was
// needed. That is not hypothetical — it was exploitable in the intended deployment topology.
//
// It is closed by `capture-digest-inventory.json`: a COMMITTED index of the sha256 and byte count
// of every raw capture, with no payload, generated by `scripts/build-capture-inventory.mjs`. The
// verifier loads it on every run, with or without the local roots, and:
//
//   * a missing, unreadable, unparseable or malformed inventory is a `capture` violation and the
//     build FAILS. It is never a skipped check — that is the whole failure mode being fixed;
//   * the inventory's digests are unioned into the capture index, so the byte-identity test has
//     the same strength on a machine that has never held a capture;
//   * when a capture root IS present, the inventory is reconciled against it by file count and
//     size multiset (stat only, no digests), so a capture added or replaced without regenerating
//     the inventory fails the build here rather than silently narrowing the check;
//   * capture findings remain outside CLEARABLE_CHECKS, so no manifest entry can clear one.
//
// WHAT THIS DOES NOT DETECT — stated here so nobody quotes the verifier for more than it proves.
// It is a byte-identity boundary. It cannot establish AUTHORSHIP (a promotion entry is the
// founder's assertion plus a receipt that the cited provenance documents have not changed), and it
// cannot see a TRANSFORMED capture: re-encode a screenshot, resample a height field, re-quantise a
// control map, and every digest changes and every hash check goes quiet. No heuristic is attempted
// for that class, deliberately — the control for it is the closed registry in
// `scripts/lib/asset-promotion.mjs` plus review of whatever writes `public/`. The full statement
// lives at the top of that module and is echoed in this verifier's report under `coverage`.
//
...
/**
 * The verifier's own statement of what it proves. Emitted in the report so a reader who quotes the
 * report cannot accidentally quote more coverage than exists. See the header comment.
 */
export const BOUNDARY_COVERAGE = Object.freeze({
  proves: Object.freeze([
    'no file in dist/ is byte-identical to a raw capture, using a committed digest inventory that '
      + 'does not require the local roots to be present',
    'no file in dist/ is byte-identical to a local package file unless the promotion manifest '
      + 'admits that exact path with that exact digest and a receipt that verifies',
    'no file in dist/ embeds an absolute EFT-install path or a local root name (UTF-8 or UTF-16LE)',
    'no capture package directory name, EFT screenshot file name, or symbolic link appears in dist/',
    'the committed capture inventory is not stale against whichever capture roots exist here',
  ]),
  doesNotProve: Object.freeze([
    'AUTHORSHIP: a promotion entry is the founder\'s assertion plus a receipt that the cited '
      + 'provenance documents are unchanged. It is not evidence of who authored the bytes.',
    'A TRANSFORMED OR RE-ENCODED CAPTURE: any re-save, resample, re-quantisation or repack changes '
      + 'the digest and every hash check here goes quiet. No perceptual or statistical heuristic is '
      + 'attempted, deliberately — the control for that class is the closed PROMOTABLE_SOURCES '
      + 'registry and review of whatever writes public/, not this verifier.',
  ]),
});
...
export async function verifyBuildBoundary({
  ...
  const inventoryPath = captureInventoryPath === undefined
    ? (repoRoot === PROMOTION_REPOSITORY_ROOT
      ? CAPTURE_INVENTORY_PATH
      : resolve(repoRoot, 'capture-digest-inventory.json'))
    : resolve(captureInventoryPath);
  ...
  // A file either sweep could not vouch for is reported, never dropped. `capture` for the capture
  // roots (non-clearable) and `read` for the intermediates — both outside CLEARABLE_CHECKS, so a
  // promotion entry cannot excuse an unreadable local file either.
  for (const issue of captureIndex.issues) {
    violations.push(violation('capture', issue.path, `capture root ${issue.subtree}: ${issue.reason}`));
  }
  for (const issue of intermediateIndex.issues) {
    violations.push(violation('read', issue.path, `intermediate root ${issue.subtree}: ${issue.reason}`));
  }

  // --- the committed capture-digest inventory. FAIL CLOSED. -----------------------------------
  //
  // This is what makes check 4 work on a clean CI checkout, where every capture root is absent and
  // `captureIndex.hashes` is empty. Absent / unparseable / malformed is a build failure: a
  // boundary that quietly downgrades to "nothing to compare against" is the exact defect this
  // replaces.
  const inventoryRelative = relative(repoRoot, inventoryPath).split(sep).join('/');
  const inventoryDocument = await readCaptureInventory(inventoryPath);
  let captureInventory = null;
  if (!inventoryDocument.present) {
    violations.push(violation(
      'capture',
      inventoryRelative,
      'the committed capture-digest inventory is missing; without it the capture check cannot run '
      + 'on a checkout that has no local roots. Regenerate it with `npm run build:capture-inventory`',
    ));
  } else if (inventoryDocument.parseError !== null) {
    violations.push(violation(
      'capture',
      inventoryRelative,
      `the committed capture-digest inventory is not valid JSON: ${inventoryDocument.parseError}`,
    ));
  } else {
    const validation = validateCaptureInventory(inventoryDocument.value, { path: inventoryRelative });
    for (const error of validation.errors) {
      violations.push(violation('capture', error.path, `ERR_CAPTURE_INVENTORY: ${error.message}`));
    }
    captureInventory = validation.value;
  }

  // Union the inventory's digests into the capture index. On a clean checkout this IS the index.
  if (captureInventory) {
    for (const [digest, source] of captureInventory.digests) {
      if (!captureIndex.hashes.has(digest)) captureIndex.hashes.set(digest, source);
    }
    for (const drift of reconcileCaptureInventory(captureInventory, captureIndex)) {
      violations.push(violation(
        'capture',
        inventoryRelative,
        `is stale against capture root ${drift.subtree}: ${drift.detail}. Regenerate it with `
  ...
  return {
    ...
    capture: {
      ...
      // The two independent sources of capture digests, reported separately on purpose. On Vercel
      // `subtreesPresent` is `[]` and `inventoryDigests` is the whole check; if that number is
      // ever 0 or `inventoryLoaded` false, the check is not running and the build has failed.
      inventory: inventoryRelative,
      inventoryLoaded: captureInventory !== null,
      inventoryCaptures: captureInventory?.captureCount ?? 0,
      inventoryDigests: captureInventory?.digests.size ?? 0,
      knownDigests: captureIndex.hashes.size,
    },
...
function parseArguments(argv) {
  const options = {
    ...
    captureInventoryPath: undefined,
    help: false,
  };
  const PATH_ARGUMENTS = new Set([
    '--dist-dir', '--local-root', '--repository-root', '--capture-inventory',
  ]);
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (!PATH_ARGUMENTS.has(argument)) {
    ...
    } else if (argument === '--capture-inventory') {
      options.captureInventoryPath = resolved;
    } else {
  ...
  if (options.captureInventoryPath === undefined) delete options.captureInventoryPath;
  return options;
}
