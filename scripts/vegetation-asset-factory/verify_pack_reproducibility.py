...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument("--sample", action="append", required=True, help="PROTOTYPE:LOD")
    ...
    require(1 <= len(args.sample) <= 8, "use 1..8 reproducibility samples")
    normalized = []
...
def main() -> None:
    ...
    for prototype, lod in args.sample:
        ...
        original = (args.pack_root / record["glb"]).resolve()
        original_receipt = (args.pack_root / record["receipt"]).resolve()
        ...
        invocation = command(args.blender, prototype, lod, record["seed"], repeat, repeat_receipt)
        ...
        no_clobber = subprocess.run(
            command(args.blender, prototype, lod, record["seed"], original, original_receipt),
            cwd=REPO,
        ...
        require(original.stat().st_size == before_size and sha256_file(original) == original_hash, f"no-clobber attempt mutated {prototype}:LOD{lod}")
        results.append({
            "prototype": prototype,
            "lod": lod,
            "seed": record["seed"],
            "original": record["glb"],
            "repeat": repeat.relative_to(args.pack_root).as_posix(),
    ...
    print(json.dumps({"output": str(args.output), "samples": results}, indent=2, sort_keys=True))
