...
7. Terrain: `scripts/ingest-elevation.mjs` merges companion survey logs, SPT spawns, and compact loose-loot positions on a 2 m dedupe grid. The builder rejects mapped building/rock/underground evidence and ±2.5 m local-median outliers, then fits a smooth 5 m multi-scale surface. `TERRAIN_FEATURES` are sparse-area fallback only. The 3D View defaults to 3× vertical relief (selectable at 1×/2×/3×); the runtime applies that factor once to the shared `H(x,z)` field before building the mesh, hillshade, flat limit skirt, and contours so every draped feature stays registered. Object heights remain real scale. See `docs/plans/ELEVATION.md` for survey runs.
