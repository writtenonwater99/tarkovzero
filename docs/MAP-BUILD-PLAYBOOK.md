...
| tarkov.dev JSON `https://json.tarkov.dev/regular/maps` + `/maps_en` | exact source objects for extracts/transits/locks/switches/hazards, loot, weapons, BTR stops, artillery, plus names | fetch only with `scripts/fetch-map-primitives.mjs`; the committed per-map gzip fixture is content-addressed and the manifest records compressed/decoded SHA-256 plus upstream provenance. Builders are cache-only and verify both hashes before parsing. |
...
7. Terrain: exact-cache positions, every SPT spawn/loose-loot position, and reviewed companion evidence enter one typed router with their original `y`. Every observation lands in exactly one `ground`, `road`, `bridge-deck`, `water`, `rock`, `floor`, `roof`, or `underground` bucket with reason codes. Only trusted ground and at-grade road observations feed the 2 m-deduped smooth 5 m heightfield. Bridge decks, water, rocks, floors, roofs, and underground surfaces remain independent hard-surface evidence and must never bend or borrow the terrain fit. Generated horizontal and vertical data is canonical 1× metres. The 3D View's selectable 1×/2×/3× relief is the sole runtime sampler transform; it is never baked into JSON. See `docs/plans/ELEVATION.md`.
...
9. Output one JSON; `src/map3d.js` renders the production view and `H(x,z)`/`Pg()` drape terrain-relative objects. Measured `surfaceY` values are absolute hard-surface altitudes, not terrain lifts. The localhost Customs Three.js proof consumes the same JSON and callback contracts; audited authored assets enter through `public/assets/3d/customs/scene-manifest.json`. The raw exact layer is embedded in `<map>-3d.json`; `<map>.json` contains renderer projections plus cache provenance.
...
```bash
node scripts/fetch-map-primitives.mjs --date YYYY-MM-DD --map customs
...
* `npm run test:map-pipeline` performs a cache-only isolated Customs rebuild and byte-compares both generated artifacts, then checks exact projection, coordinate-free Wiki quarantine, typed surface routing, canonical 1× data, and relief invariants. It also protects the untouched Reserve/Woods artifacts with read-only regressions.
* `npm run test:map-accuracy-harness` tests the independent-evidence contract. `npm run audit:customs` is the real certification gate and must exit nonzero until reviewed first-party train and held-out observations satisfy every horizontal, vertical, coverage, surface, bounds, and object-fidelity threshold.
