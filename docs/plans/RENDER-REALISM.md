...
- **Cameras:** JSON bookmarks containing exact OrbitView `target`, `zoom`, `rotationX`, and `rotationOrbit`. Minimum suite: `customs-wide`, `customs-mid`, `customs-close`, `reserve-wide-courtyard`, `reserve-courtyard`, `woods-wide`, `woods-forest-edge`, `woods-mountain-lake` (`scripts/render-baseline.mjs`). Customs is a **wide / mid / close ladder** and each rung must be at least a zoom level from the next: wide is the app's own default framing, mid is `#3.6/231/150` (Dorms 2-Story), and **close is `#5/203/-128` (the Fortress) — no "close" rung may sit within a zoom level of the default**, or the material stages have nothing to be judged on. Every row records `honoured`: the camera clamp rewrites the hash when it moves a permalink, so a bookmark that no longer frames what it says fails loudly instead of measuring some other camera.
